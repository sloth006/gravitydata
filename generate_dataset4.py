...
Generate a fixed subset of GQA datasets where:
  - q=1 and q=5, but skip (kv_mean=1, q_len=5)
...
def _upload_csv_to_drive_replace(
    ...
    try:
        ...
        print(f"Warning: Failed to delete existing Drive file {local_path.name}: {e}")
            ...
            for kv_mean in KV_MEANS:
                ...
                for kv_dist_type, dist_key in dists:
                    for q_len in Q_LENGTHS:
                        # Exclude the requested invalid workload.
                        if kv_mean == 1 and q_len == 5:
                            continue
                        name = _filename(
                            DTYPE,
                            kv_dist_type,
...
def run(
    ...
    *,
    ...
    def is_pending(row: dict) -> bool:
        ...
        # Never retry known-bad jobs (prevents infinite OOM loops).
        ...
        for job_idx, row in enumerate(rows):
            ...
            try:
                ...
                if torch.cuda.is_available():
                    # Start from a clean slate before the heavy allocations.
                ...
                # Delete references BEFORE empty_cache to avoid a phantom leak cascade.
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate GQA subset with Drive + resume.")
